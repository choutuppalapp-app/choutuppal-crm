...
class MyUtils {
  ...
  static SystemUiOverlayStyle allScreen() {
    return SystemUiOverlayStyle(statusBarColor: MyColor.getPrimaryColor(), statusBarIconBrightness: Brightness.dark, systemNavigationBarColor: MyColor.white, systemNavigationBarIconBrightness: Brightness.dark);
  }

  static dynamic getShadow({Color? color, Offset? offset, double? blurRadius, double? spreadRadius}) {
    return [BoxShadow(blurRadius: blurRadius ?? 15.0, offset: offset ?? const Offset(0, 25), color: color ?? MyColor.getBackgroundColor().withValues(alpha: 0.6), spreadRadius: spreadRadius ?? -35.0)];
  }

  static dynamic getShadow2({double blurRadius = 8, Color? color, Offset? offset, double? spreadRadius}) {
    return [BoxShadow(blurRadius: blurRadius, offset: offset ?? const Offset(0, 25), color: color ?? MyColor.getBackgroundColor().withValues(alpha: 0.6), spreadRadius: spreadRadius ?? -35.0), BoxShadow(blurRadius: blurRadius, offset: offset ?? const Offset(0, 1), color: color ?? MyColor.getBackgroundColor().withValues(alpha: 0.6), spreadRadius: spreadRadius ?? 1)];
  }

  static dynamic getBottomSheetShadow() {
    return [BoxShadow(color: MyColor.getBackgroundColor().withValues(alpha: 0.08), spreadRadius: 3, blurRadius: 4, offset: const Offset(0, 3))];
  }
  ...
  static dynamic getCardShadow() {
    return [BoxShadow(color: MyColor.getBackgroundColor().withValues(alpha: 0.05), spreadRadius: 2, blurRadius: 2, offset: const Offset(0, 3))];
  }
  ...
  static String getFileType(String path) {
    ...
    if (lowerPath.endsWith('.jpg') || lowerPath.endsWith('.jpeg') || lowerPath.endsWith('.png') || lowerPath.endsWith('.gif') || lowerPath.endsWith('.bmp') || lowerPath.endsWith('.webp')) {
      return 'image';
    }
    ...
    if (lowerPath.endsWith('.mp4') || lowerPath.endsWith('.avi') || lowerPath.endsWith('.mov') || lowerPath.endsWith('.wmv') || lowerPath.endsWith('.flv') || lowerPath.endsWith('.mkv') || lowerPath.endsWith('.webm') || lowerPath.endsWith('.3gp') || lowerPath.endsWith('.mpeg')) {
      return 'video';
    }
  ...
  static bool checkPermission(String permissionName) {
    return Get.find<DashboardController>().permissionList.contains(permissionName) || Get.find<DashboardController>().user?.isAgent == "0";
  }
}
