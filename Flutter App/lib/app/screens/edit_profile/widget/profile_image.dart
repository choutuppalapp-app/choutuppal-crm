...
class ProfileWidget extends StatefulWidget {
  ...
  const ProfileWidget({super.key, required this.imagePath, required this.onClicked, this.isEdit = false, this.onEditTap});
...
class _ProfileWidgetState extends State<ProfileWidget> {
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: widget.isEdit == true ? MyColor.getPrimaryColor() : MyColor.notificationLineColor),

                borderRadius: BorderRadius.circular(Dimensions.space12.r),
              ),
              child: DefaultText(
                text: widget.isEdit == true ? MyStrings.edit.tr : MyStrings.cancel.tr,
                textStyle: MyTextStyle.heading16W600().copyWith(color: widget.isEdit == true ? MyColor.white : MyColor.dark, fontWeight: FontWeight.w500),
              ),
            ),
          ),
        ),
  ...
  Widget _buildProfileImageContent() {
    ...
    if (widget.imagePath.isNotEmpty && widget.imagePath.startsWith('http')) {
      return MyNetworkImageWidget(isProfile: true, imageUrl: widget.imagePath, height: 100, width: 100, radius: 100, boxFit: BoxFit.cover);
    }

    // 3️⃣ Asset fallback
    return CircleImageWidget(onTap: widget.onClicked, isAsset: true, imagePath: MyImages.profile, height: 100, width: 100);
  }
