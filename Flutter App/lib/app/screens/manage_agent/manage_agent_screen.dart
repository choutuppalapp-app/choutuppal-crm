...
class _ManageAgentScreenState extends State<ManageAgentScreen> {
  ...
  Widget buildAgentListCard(ManageAgentController controller, int index, ThemeData theme, BuildContext context, bool isLastIndex) {
    return Column(
      children: [
        Container(
          ...
          child: ListTile(
            leading: controller.agentList[index].image != null ? CircleAvatar(maxRadius: 23, backgroundImage: NetworkImage("${UrlContainer.domainUrl}/${controller.imagePath}/${controller.agentList[index].image}")) : AlphabetAvatar(firstname: controller.agentList[index].firstname?.substring(0, 2).toUpperCase() ?? "", lastName: controller.agentList[index].lastname?.substring(0, 2).toUpperCase() ?? ''),
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: theme.textTheme.headlineSmall?.copyWith(color: MyColor.black, fontSize: 15, fontWeight: FontWeight.w700),
                      ),
                      spaceDown(Dimensions.space4.h),
                      Text(
                        controller.agentList[index].username ?? "",
                        style: theme.textTheme.labelMedium?.copyWith(color: MyColor.darkSecondary, fontSize: 14, fontWeight: FontWeight.w500),
                      ),
                    ],
                  ),
                ),
                Row(
                  children: [
                    ...
                    PopupMenuButton<int>(
                      itemBuilder: (context) {
                        if (!MyUtils.checkPermission(AppPermission.editAgent) && !MyUtils.checkPermission(AppPermission.deleteAgent) && !MyUtils.checkPermission(AppPermission.viewPermission)) {
                          [
                        ...
                        return [
                          ...
                          if (MyUtils.checkPermission(AppPermission.editAgent))
                            PopupMenuItem(
                              onTap: () {
                                Get.toNamed(RouteHelper.editAgentScreen, arguments: controller.agentList[index])?.then((value) {
                                  controller.initData(initPage: true);
                                });
                              },
