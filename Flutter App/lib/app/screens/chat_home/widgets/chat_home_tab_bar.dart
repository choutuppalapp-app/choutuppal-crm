        ...
        return NotificationListener<ScrollNotification>(
            ...
            if (notification is ScrollUpdateNotification) {
              ...
              if (metrics.pixels >= metrics.maxScrollExtent - 100) {
                if (!controller.isLoadingMore && controller.hasNext()) {
                  controller.newChatMethod(loadMore: true, status: controller.status, searchQuery: controller.searchQuery);
                }
              }
            }
            return false;
          ...
          child: CustomScrollView(
            slivers: [
                            ...
                            children: [
                              item.contact?.imageSrc != null
                                  ? Padding(
                                      ...
                                      child: MyNetworkImageWidget(boxFit: BoxFit.fill, height: 54.h, width: 54.w, radius: 100, imageUrl: item.contact?.imageSrc ?? ''),
                                    )
                                  : Padding(
                                      padding: EdgeInsets.only(top: Dimensions.space5),
                                      child: AlphabetAvatar(firstname: item.contact?.firstname ?? "", lastName: item.contact?.lastname ?? "", size: 54.w),
                                    ),
                              (int.tryParse(item.unseenMessages ?? "0") ?? 0) > 0
                                  ? Positioned(
                                      ...
                                      child: Container(
                                        ...
                                        child: Text(
                                          (int.tryParse(item.unseenMessages ?? "0") ?? 0) > 99 ? "99+" : (item.unseenMessages ?? "0"),
                                          style: MyTextStyle.subHeading15W500FieldTitleColor.copyWith(fontSize: Dimensions.space12.sp, color: MyColor.white),
                                        ),
                                      ),
                                    )
                              ...
                              children: [
                                  ...
                                  children: [
                                    ...
                                    if (item.lastMessageAt != null)
                                      DefaultText(
                                        ...
                                        textStyle: MyTextStyle.subHeading12W600().copyWith(color: MyColor.getBodyTextColor()),
                                        maxLines: 1,
                                      ),
                                  ],
                                ),
                                ...
                                if (item.lastMessage?.message != null) ...[
                                  ...
                                  DefaultText(
                                    ...
                                    textStyle: MyTextStyle.subHeading14W400().copyWith(color: MyColor.planStatusTextColor),
                                    maxLines: 1,
                                  ),
                                ] else ...[
                                  ...
                                  children: [
                                    if (controller.tabController.index == 0) ...[
                                      ...
                                      (tag?.isNotEmpty == true)
                                          ? Container(
                                              padding: EdgeInsets.symmetric(horizontal: Dimensions.space8.w, vertical: Dimensions.space4.h),
                                              decoration: BoxDecoration(color: MyColor.splashScreenBackground, borderRadius: BorderRadius.circular(Dimensions.space10.r)),
                                              child: DefaultText(
                                                text: tag ?? ''.tr,
                                                textStyle: MyTextStyle.subHeading12W600().copyWith(color: MyColor.splashTextColor),
                                              ),
                                            )
                                          : const SizedBox.shrink(),
                                    ],
              ...
              SliverToBoxAdapter(child: controller.isLoadingMore ? CustomLoader(isPagination: true, loaderSize: 50.sp) : SizedBox.shrink()),
            ],
          ),
        );
      },
