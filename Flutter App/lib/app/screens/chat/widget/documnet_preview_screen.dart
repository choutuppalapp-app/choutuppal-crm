      ...
      actionButton: [
        if (!isLoading && !hasError && totalPages > 0)
          Container(
            ...
            child: Center(
              child: Text('${currentPage + 1} / $totalPages', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            ),
          ),
      ],
      body: _buildBody(),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton.icon(onPressed: downloadAndSavePDF, icon: const Icon(Icons.refresh), label: const Text('Retry')),
          ],
        ),
      );
    }
  ...
  Widget? _buildBottomNavigation() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        boxShadow: [BoxShadow(color: Colors.grey.withAlpha((0.3 * 255).round()), spreadRadius: 1, blurRadius: 5, offset: const Offset(0, -2))],
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: [
          IconButton(onPressed: currentPage > 0 ? _goToPreviousPage : null, icon: const Icon(Icons.arrow_back_ios), tooltip: 'Previous Page'),
          Expanded(
            child: Slider(
          ...
          IconButton(onPressed: currentPage < totalPages - 1 ? _goToNextPage : null, icon: const Icon(Icons.arrow_forward_ios), tooltip: 'Next Page'),
        ],
      ),
    );
  }
