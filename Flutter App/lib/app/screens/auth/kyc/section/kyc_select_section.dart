...
class _KycSelectSectionState extends State<KycSelectSection> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        LabelTextInstruction(text: (widget.model.name ?? '').tr.capitalize!, isRequired: widget.model.isRequired == 'optional' ? false : true, instructions: widget.model.instruction),
        const SizedBox(height: Dimensions.textToTextSpace),
        CustomDropDownWithTextField(borderWidth: .5, list: widget.model.options ?? [], onChanged: (value) => widget.onChanged(value), selectedValue: widget.model.selectedValue),
