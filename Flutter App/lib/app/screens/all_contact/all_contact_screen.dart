    ...
    return MyCustomScaffold(
      ...
      appBarContent: GetBuilder<AllContactController>(
        builder: (controller) => Row(
          children: [
            Expanded(
              child: Text(MyStrings.allContacts.tr, style: theme.textTheme.headlineMedium?.copyWith(color: MyColor.getHeadingTextColor())),
            ),
            GestureDetector(
              onTap: () {
                CustomAlertDialog(verticalPadding: 0, isHorizontalPadding: false, child: UploadCsvDialogue()).customAlertDialog(context);
              },
              child: Container(
                padding: EdgeInsets.all(8),
                decoration: BoxDecoration(color: MyColor.getInformationColor().withValues(alpha: .7), borderRadius: BorderRadius.circular(Dimensions.space5)),
            ...
            Visibility(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  padding: EdgeInsets.all(8),
                  decoration: BoxDecoration(color: MyColor.getPrimaryColor(), borderRadius: BorderRadius.circular(Dimensions.space5)),
            ...
            children: [
              LabelTextField(
                ...
                prefixIcon: Padding(
                  ...
                  child: MyAssetImageWidget(assetPath: MyImages.search, isSvg: true, height: Dimensions.space10.h, width: Dimensions.space10.h),
                ),
                validator: (value) {
                  if (value!.isEmpty) {
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          if (controller.allContactsData.length == index) {
                            return controller.hasNext() ? Center(child: CircularProgressIndicator(color: MyColor.getPrimaryColor())) : const SizedBox();
                          }
                          var item = controller.allContactsData[index];
                          return Container(
                            padding: const EdgeInsets.only(top: 5, bottom: 5, right: 5),
                            margin: const EdgeInsets.only(bottom: 10),
                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(4), color: MyColor.getCardBackgroundColor()),
                            child: ListTile(
                              contentPadding: EdgeInsets.symmetric(horizontal: Dimensions.space10.w),
                              leading: item.image != null ? CircleAvatar(maxRadius: 25, backgroundImage: NetworkImage("${UrlContainer.domainUrl}/${controller.imagePath}/${item.image}")) : AlphabetAvatar(firstname: item.firstname ?? "", lastName: item.lastname ?? ""),
                              title: Row(
                                children: [
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        Text("${item.firstname ?? ""} ${item.lastname ?? ""}", style: theme.textTheme.headlineSmall?.copyWith(color: MyColor.black)),
                                        spaceDown(Dimensions.space4.h),
                                        Text("+${item.mobileCode ?? ""}${item.mobile ?? ""}", style: theme.textTheme.labelMedium?.copyWith(color: MyColor.getBodyTextColor())),
                                      ],
                                    ),
                                  ),
                                  Row(
                                    children: [
                                      InkWell(
                                        onTap: () {
                                          Get.toNamed(RouteHelper.customerAccountScreen, arguments: [controller.imagePath, item, true]);
                                        },
                                        child: MyAssetImageWidget(assetPath: MyImages.edit, isSvg: true, height: Dimensions.space20.h, width: Dimensions.space20.h, color: MyColor.getPrimaryColor()),
                                      ),
                                      ...
                                      InkWell(
                                        ...
                                        child: MyAssetImageWidget(assetPath: MyImages.send, isSvg: true, height: Dimensions.space20.h, width: Dimensions.space20.h, color: MyColor.getInformationColor()),
                                      ),
                                      spaceSide(Dimensions.space10),
                                      Visibility(
                                        ...
                                        child: InkWell(
                                          ...
                                          child: MyAssetImageWidget(assetPath: MyImages.delete, isSvg: true, height: Dimensions.space20.h, width: Dimensions.space20.h, color: MyColor.getErrorColor()),
                                        ),
                                      ),
                                    ],
                                  ),
