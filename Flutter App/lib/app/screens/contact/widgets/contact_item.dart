        ...
        return Column(
          children: [
            Row(
              ...
              children: [
                if (item.image?.isNotEmpty ?? false) ...[MyNetworkImageWidget(imageUrl: '${item.imageSrc}', height: 44.h, width: 44.w, radius: 22.r)] else ...[AlphabetAvatar(firstname: item.firstname ?? "", lastName: item.lastname ?? '')],
                      ...
                      Row(
                        children: [
                          ...
                          if (controller.tabController.index == 0) ...[
                            Container(
                              padding: EdgeInsets.symmetric(horizontal: Dimensions.space6.w, vertical: Dimensions.space2.h),
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(Dimensions.space7.r),
                                border: Border.all(color: item.status == '1' ? MyColor.campaignsRunning.withAlpha(MyColor.getAlpha(10)) : MyColor.splashTextColor.withAlpha(MyColor.getAlpha(10))),
                                color: item.status == '1' ? MyColor.campaignsRunning.withAlpha(MyColor.getAlpha(10)) : MyColor.splashTextColor.withAlpha(MyColor.getAlpha(10)),
                              ),
                              child: DefaultText(
                                text: item.status == '1' ? MyStrings.active.tr : MyStrings.inactive.tr,

                                textStyle: MyTextStyle.subHeading12W400().copyWith(color: item.status == '1' ? MyColor.campaignsRunning : MyColor.splashTextColor),
                              ),
                            ),
                          ],
                        ],
                ...
                PopupMenuButton<int>(
                  itemBuilder: (context) {
                    if (!MyUtils.checkPermission(AppPermission.editContact) && !MyUtils.checkPermission(AppPermission.deleteContact) && !MyUtils.checkPermission(AppPermission.sendMessage)) {
                      ...
                      if (MyUtils.checkPermission(AppPermission.editContact))
                        PopupMenuItem(
                          onTap: () {
                            ...
                            Get.toNamed(RouteHelper.customerAccountScreen, arguments: [item.imageSrc, item, true, index, false]);
                          },
