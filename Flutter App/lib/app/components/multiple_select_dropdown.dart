...
class TagSelectorState extends State<TagSelector> {
  ...
  void _setupController() {
    final dropdownItems = widget.tags.map((tag) => DropdownItem<AllContacts>(value: tag, label: tag.name ?? "")).toList();
  ...
  Widget build(BuildContext context) {
    return MultiDropdown<AllContacts>(
      ...
      chipDecoration: ChipDecoration(backgroundColor: MyColor.lightPrimary.withValues(alpha: .2), wrap: true, runSpacing: 2, spacing: 10),
