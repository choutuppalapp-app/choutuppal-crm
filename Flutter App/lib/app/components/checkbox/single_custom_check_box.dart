...
class SingleCustomCheckbox extends StatelessWidget {
  ...
  const SingleCustomCheckbox({super.key, required this.value, required this.onChanged, this.isRememberMeTextShow = false, this.checkText = MyStrings.rememberMe, this.textStyle});
    ...
    return Row(
      children: [
        ...
        isRememberMeTextShow
            ? Text(
                checkText,
                style: textStyle ?? MyTextStyle.subHeading14W400().copyWith(color: MyColor.regularHederColor, fontSize: 14.sp),
              )
            : SizedBox.shrink(),
      ],
    );
