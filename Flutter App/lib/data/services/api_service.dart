...
class ApiService {
  ...
  static Future<ResponseModel> getRequest(String endpoint, {Map<String, dynamic>? params}) async {
    try {
      ...
      return ResponseModel(isSuccess: true, message: response.data.toString(), statusCode: response.statusCode ?? 200, responseJson: response.data);
    } on DioException catch (e) {
      printE('GET request error: ${e.response?.statusCode}, ${e.message}');
      return ResponseModel(isSuccess: false, message: e.message ?? 'Error occurred', statusCode: e.response?.statusCode ?? 500, responseJson: e.response?.data.toString() ?? '');
    }
  }
  ...
  static Future<ResponseModel> downloadFile(String url, String savePath) async {
    try {
      ...
      return ResponseModel(isSuccess: true, message: "File downloaded successfully", statusCode: response.statusCode ?? 200, responseJson: response.data);
    } on DioException catch (e) {
      printE('Download error: ${e.message}');
      return ResponseModel(isSuccess: false, message: e.message ?? 'Error occurred during file download', statusCode: e.response?.statusCode ?? 500, responseJson: e.response?.data.toString() ?? '');
    }
  }
}
