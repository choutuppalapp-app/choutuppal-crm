...
class PusherManager {
  ...
  Future<void> init(String channelName, {required String apiKey, required String cluster, required Future<dynamic> Function(String channelName, String socketId, dynamic options) onAuthorizer, Function(String message, int? code, dynamic e)? onError, Function(String message, dynamic e)? onSubscriptionError, Function(String channelName, dynamic data)? onSubscriptionSucceeded}) async {
    await pusher.init(apiKey: apiKey, cluster: cluster, onConnectionStateChange: (_, _) {}, onEvent: _dispatchEvent, onError: onError, onSubscriptionError: onSubscriptionError, onSubscriptionSucceeded: onSubscriptionSucceeded, onAuthorizer: onAuthorizer, onDecryptionFailure: (_, _) {}, onMemberAdded: (_, _) {}, onMemberRemoved: (_, _) {});
  ...
  Future<void> checkAndInitIfNeeded(String channelName) async {
    ...
    if (state.toLowerCase() == 'disconnected' || state == 'disconnecting' || state == 'connecting') {
      ...
      final apiKey = SharedPreferenceService.getGeneralSettingData().data?.generalSetting?.pusherConfig?.pusherAppKey ?? "";
      final cluster = SharedPreferenceService.getGeneralSettingData().data?.generalSetting?.pusherConfig?.pusherAppCluster ?? "";
      printE(apiKey);
      printE(cluster);
      await init(
        channelName,
