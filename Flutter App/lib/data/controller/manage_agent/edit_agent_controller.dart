...
class EditAgentController extends GetxController {
  ...
  void loadData(AgentData? agent) async {
    ...
    // selectACountry();
    countryData = Countries(countryCode: agent?.countryCode ?? "", country: agent?.countryName ?? "", dialCode: agent?.dialCode ?? "");
    update();
  }
  ...
  void updateAgent() async {
    ...
    update();
    try {
      Map<String, dynamic> pram = {"firstname": firstNameController.text.trim(), "lastname": lastNameController.text.trim(), "city": cityController.text.trim(), "state": stateController.text.trim(), "zip": zipCodeController.text.trim(), "address": addressController.text.trim()};
