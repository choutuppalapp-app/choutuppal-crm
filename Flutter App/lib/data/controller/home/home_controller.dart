...
class HomeController extends GetxController {
    ...
    try {
      ...
      if (model.statusCode == 200) {
        ...
        if (responseModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
          ...
          SharedPreferenceService.setString(SharedPreferenceService.userNameKey, responseModel.data?.user?.username ?? "");
    ...
    try {
      ...
      if (model.statusCode == 200) {
        ...
        if (responseModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
          ...
          selectedNumber = whatsappNumbers.firstWhereOrNull((e) => e.isDefault == '1')?.phoneNumber ?? MyStrings.connectAccount.tr;
  ...
  void filterChats() {
    if (searchQuery.isEmpty) {
      ...
      filteredAllChats = allChatsData.where((chat) => "${chat.contact?.firstname ?? ''}${chat.contact?.lastname ?? ''}".toLowerCase().contains(searchQuery)).toList();
      filteredPendingChats = pendingChatsData.where((chat) => "${chat.contact?.firstname ?? ''}${chat.contact?.lastname ?? ''}".toLowerCase().contains(searchQuery)).toList();
      filteredOpenChats = openChatsData.where((chat) => "${chat.contact?.firstname ?? ''}${chat.contact?.lastname ?? ''}".contains(searchQuery)).toList();
      filteredSolvedChats = solvedChatsData.where((chat) => "${chat.contact?.firstname ?? ''}${chat.contact?.lastname ?? ''}".contains(searchQuery)).toList();
    }
    update();
  }
  ...
  Future<void> newChatMethod({bool loadMore = false, String? searchQuery, int? status, bool shouldShowLoader = true}) async {
    try {
      if (loadMore) {
        isLoadingMore = true;
        page++;
      ...
      if (model.statusCode == 200) {
        ...
        if (responseModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
          ...
          await homeData().then((v) {
            Get.find<PusherHomeServiceController>().ensureConnection("private-receive-message-$selectedWpAccountNumber");
          });
        } else {
