...
class AllContactListController extends GetxController {
  ...
  void pickFile() async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(allowMultiple: false, type: FileType.custom, allowedExtensions: ['csv', 'xlsx']);
  ...
  void submitContactData() async {
    ...
    try {
      ...
      if (model.statusCode == 200) {
        ...
        if (responseModel.status?.toLowerCase() == AppStatus.success) {
          if (id == "") {
            allContactListdata.insert(0, AllContactData(name: responseModel.data?.data?.name ?? "", contact: [], id: responseModel.data?.data?.id, userId: responseModel.data?.data?.userId));
          } else {
            int index = allContactListdata.indexWhere((c) => c.id == id);
            if (index != -1) {
              allContactListdata[index] = AllContactData(name: responseModel.data?.data?.name ?? "", contact: [], id: responseModel.data?.data?.id, userId: responseModel.data?.data?.userId);
            }
          }
