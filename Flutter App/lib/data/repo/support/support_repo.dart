...
class SupportRepo {
  ...
  Future<dynamic> storeTicket(TicketStoreModel model) async {
    ...
    Map<String, File> attachmentFiles = model.list!.asMap().map((index, value) => MapEntry("attachments[$index]", value));
  ...
  Future<dynamic> replyTicket(String message, List<File> fileList, String ticketId) async {
    try {
      ...
      Map<String, File> attachmentFiles = fileList.asMap().map((index, value) => MapEntry("attachments[$index]", value));
