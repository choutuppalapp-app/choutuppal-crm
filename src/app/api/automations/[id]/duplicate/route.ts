...
export async function POST(
  ...
  // Account-scoped, not author-scoped — any agent+ teammate may
  // duplicate an automation on the account, not just its creator.
  ...
  const { data: copy, error: copyErr } = await admin
    ...
    .insert({
      ...
      // Audit: the copy is attributed to whoever duplicated it, not the
      // original automation's author.
      user_id: userId,
