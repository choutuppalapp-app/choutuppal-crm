...
-- UUID defaults use gen_random_uuid(), which lives in pg_catalog on
-- Postgres 13+ and therefore needs no extension and no search_path
-- entry. uuid-ossp's uuid_generate_v4() was used here originally, but
-- hosted Supabase installs extensions into the `extensions` schema,
-- which is not on the migration role's search_path — so unqualified
-- calls to it fail during `supabase db push`.
